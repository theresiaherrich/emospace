...
import { type dataSpecialist, type ConsultantAPI } from "../types/type";
import { getSpecialist } from "../../../services/specialistservice";

const SpecialistContainer: React.FC = () => {
  const [searchQuery, setSearchQuery] = useState("");
  const [allData, setAllData] = useState<dataSpecialist[]>([]);
  ...
  const fetchSpecialists = async () => {
    try {
        ...
        const mapped = res.map((item: ConsultantAPI) => ({
        id: item.ID,
        name: item.Name,
        photo: item.ImageURL,
        job: item.Title,
        expertise: item.Speciality,
        experience: `${item.Experience} tahun`,
        rating: `${Math.round((item.Rating / 5) * 100)}%`,
        price: item.Price,
        }));
        setAllData(mapped);
        setFilteredData(mapped);
    } catch (err) {
        console.error("Gagal ambil data spesialis:", err);
    }
    };
