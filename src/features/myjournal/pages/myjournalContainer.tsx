...
const MyJournalContainer = () => {
  ...
  const [filteredJournals, setFilteredJournals] = useState<JournalAPI[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const sorted = response.sort((a: JournalAPI, b: JournalAPI) => new Date(b.date).getTime() - new Date(a.date).getTime());
        setJournals(sorted);
  ...
  }, [selectedMonth, activeFilter, journals]);
  ...
  }, [selectedYear, activeFilter, journals]);
      ...
      <div className="pt-40 px-8 sm:px-10 md:px-20 pb-7 flex flex-col min-h-screen relative z-10">
        ...
        <div className="flex flex-col gap-5 py-2">
          <div className="flex flex-wrap gap-2 justify-center sm:justify-start py-2">
            <Button variant="label" onClick={() => handleFilterClick("All")} active={activeFilter === "All"}>
              All
            </Button>
            <Button variant="label" onClick={() => handleFilterClick("Month")} active={activeFilter === "Month"}>
              Month
            </Button>
            <Button variant="label" onClick={() => handleFilterClick("Year")} active={activeFilter === "Year"}>
              Year
            </Button>
          </div>
          ...
          <div className="flex justify-end -mt-4">
            {activeFilter === "Month" && (
              <CalendarSelect value={selectedMonth} onChange={setSelectedMonth} mode="month" />
            )}
            {activeFilter === "Year" && (
              <CalendarSelect value={selectedYear} onChange={setSelectedYear} mode="year" />
            )}
          </div>
          ...
          {filteredJournals.length === 0 ? (
            <div className="text-center text-gray-500 mt-10">No journals found.</div>
          ) : (
